...
class JMX(object):
    def __init__(self, logpath, debug=False):
        ...
        self._debug = debug
        self._java_path = "/usr/java/jdk1.8.0_77/bin/java"## which("java")
    ...
    def _get_jmx_port_list(self):
        ...
        for line in stdo_list:
            ...
            runuser=ln_ary[0]
            pid = ln_ary[1]
            appname = ln_ary[-1].split("/")[-2].upper()
            gcs = []
            jmxport = None
    ...
    def get_item(self, beanstr, key, port, iphost='localhost', auth='-'):
        ...
        if not sub_key and stdo_list:
            line = stdo_list[-1]
            ln_ary = re.split(":", line.split("org.archive.jmx.Client")[-1])
            if ln_ary and len(ln_ary) >= 2:
                if pre_key in ln_ary[-2]:
                    return ln_ary[-1].strip()
